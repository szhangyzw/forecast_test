...
def run_remaining_xgb_pooled_backtest(output_dir: str | Path = DEFAULT_OUTPUT, exclude_months=None):
    ...
    for platform in sorted(all_snap['platform'].unique()):
        ...
        for target_month in complete_months:
            for cutoff in CUTOFFS:
                ...
                # pooled: all historical months, all cutoff snapshots
                train = p_all[p_all['target_month'] < target_month].copy()
                model = train_xgb(train, FEATURES_REMAINING_POOLED, 'target_remaining_sales')
                pred_remaining = predict_xgb(model, row).iloc[0]
                if pd.isna(pred_remaining):
                    continue
                pred_remaining = max(float(pred_remaining), 0.0)
                ...
                monthly_rows.append({
                    ...
                    'cutoff_day': cutoff,
                    'platform': platform,
                    'model_name': 'xgboost_remaining_pooled_v2',
    ...
    result_df = pd.DataFrame(monthly_rows).sort_values(['cutoff_day', 'platform', 'target_month']) if monthly_rows else pd.DataFrame()
    summary_df = (
    ...
    if not result_df.empty:
        result_df.to_csv(output_dir / 'remaining_xgb_pooled_backtest.csv', index=False)
    if not summary_df.empty:
        summary_df.to_csv(output_dir / 'remaining_xgb_pooled_summary.csv', index=False)

    return result_df, summary_df


if __name__ == '__main__':
    result_df, summary_df = run_remaining_xgb_pooled_backtest()
    print('=== REMAINING XGB POOLED SUMMARY ===')
